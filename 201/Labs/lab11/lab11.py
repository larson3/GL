...
def main():
    ...
    items=stack()
    item=""
    userEntry=input("Enter an expression: ")
    for currentLetter in userEntry:
        
        
        if currentLetter!=")":
            push(items,currentLetter)
            print("Pushing "+top(items)+" into stack")
        else:
            items.append(")")
            while not isEmpty(items):
                print("Popping operand "+top(items)+" from stack")
                strOutput=pop(items)+strOutput
            strOutput=str(eval(strOutput))
            push(items,strOutput)
            print("Pushing "+top(items)+" into stack")
    print(items)
    ...
    strOutput=str(eval(strOutput))
    print("Final answer is "+strOutput)
